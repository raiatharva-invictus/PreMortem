...
// src/agent-session/SessionHandle.ts
var SessionHandle_exports = {};
__export(SessionHandle_exports, {
  SessionHandle: () => SessionHandle
});
module.exports = __toCommonJS(SessionHandle_exports);
var import_schema = require("../core/events/schema.js");
var import_AgentThread = require("../core/runtime/AgentThread.js");
var import_AgentThreadOrchestrator = require("../core/runtime/AgentThreadOrchestrator.js");
var import_contextUtils = require("../core/runtime/contextUtils.js");
var import_builtinsFromSpec = require("./builtinsFromSpec.js");
...
var import_TurnHandle = require("./TurnHandle.js");
var MAX_TURN_ANCESTORS = 20;
function resolvePreviousTurnId(requested, lastTurnId) {
  if (requested === "none") {
    return null;
  }
  if (requested === void 0 || requested === "auto") {
    return lastTurnId ?? null;
  }
  return requested;
}
function toSendBatch(input) {
  if (!input || input.length === 0) {
    return [];
  }
  if (input.every((msg) => (0, import_contextUtils.isApprovalDecisionMessage)(msg) || (0, import_contextUtils.isClientSideToolResponseMessage)(msg))) {
    return input;
  }
  if (input.every(import_contextUtils.isInputUserMessage)) {
    return input;
  }
  throw new Error("input must be homogeneous: all user messages, or all approval/tool-response messages");
}
function toNewThreadInit(snapshot) {
...
function collectContextAppends(events) {
  return (async () => {
    const appendMap = /* @__PURE__ */ new Map();
    for await (const event of events) {
      const existing = appendMap.get(event.thread_id);
      if (existing) {
        existing.context.push(...event.context);
        if (event.current_context_usage !== void 0) {
...
function cancelledTurnDoneEvent(reason) {
  return {
    type: import_events.EventType.TURN_DONE,
...
function threadsRecordFromMap(threads) {
  const out = {};
...
function threadIdsForCurrentTurn(snapshot) {
  return snapshot ? Object.keys(snapshot.threads) : [import_TurnRecord.MAIN_THREAD_ID];
}
var SessionHandle = class {
  ...
  async createTurn(input) {
    ...
    const previous = previousTurnId ? await this.freezeTurn({
      ...
      reason: import_turn.CancellationReason.CancelledForNextTurn
    }) : void 0;
    const custom = typeof input.custom === "function" ? input.custom(previous?.custom ?? void 0) : input.custom;
    try {
      ...
      const sandbox = await input.resolver.resolveSandbox({
        spec,
        existing: previous?.snapshot.sandbox_info ?? void 0,
      ...
      const mainDefinition = definitionsByThreadId.get(import_TurnRecord.MAIN_THREAD_ID);
      if (!mainDefinition) {
      ...
      const orchestrator = new import_AgentThreadOrchestrator.AgentThreadOrchestrator({
        agentThreads,
      ...
      const new_threads = [];
      if (!previous) {
        const root = agentThreads.get(import_TurnRecord.MAIN_THREAD_ID);
        if (root) {
          new_threads.push(toNewThreadInit(root.toSnapshot()));
        }
      } else {
        for (const [threadId, thread] of agentThreads) {
          if (previous.snapshot.threads[threadId] === void 0) {
      ...
      return new import_TurnHandle.TurnHandle({
        store: this.store,
        turn: turnRecord,
        orchestrator,
        resolver: input.resolver,
  ...
  async getTurn(turn_id) {
    return import_TurnHandle.TurnHandle.get({
      store: this.store,
      session_id: this.session.session_id,
      turn_id
    });
  ...
  buildThread(input) {
    ...
    const capabilities = [
      ...(0, import_builtinsFromSpec.builtinsFromSpec)({
        spec: input.spec,
        definition,
        isChild,
    ...
    return new import_AgentThread.AgentThread({
      definition,
      ...
      parent: input.previousThreadSnapshot?.parent ?? void 0,
      agentInfo: input.previousThreadSnapshot?.agent_info ?? void 0,
      preComputedCompletion: input.previousThreadSnapshot?.completion ?? void 0,
      capabilities,
      capabilityState: input.previousThreadSnapshot?.capability_state ?? void 0,
      tracing: input.tracing,
      logger: input.resolver.logger
    });
  }
  makeCreateDynamicSubAgentThread(input) {
    return async (params) => {
...
//# sourceMappingURL=SessionHandle.js.map