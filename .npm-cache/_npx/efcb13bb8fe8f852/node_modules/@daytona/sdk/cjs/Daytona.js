...
const WarmPool_1 = require("./WarmPool");
const Import_1 = require("./utils/Import");
const EventDispatcher_1 = require("./utils/EventDispatcher");
const EventSubscriptionManager_1 = require("./utils/EventSubscriptionManager");
const packageJson = (0, Import_1.getPackageInfo)();
...
let Daytona = (() => {
    ...
    return class Daytona {
        static {
            ...
            _fork_decorators = [(0, otel_decorator_1.WithInstrumentation)()];
        ...
        constructor(config) {
            ...
            if (config) {
                ...
                if (config?.requestTimeoutMs !== undefined &&
                    ...
                    throw new DaytonaError_1.DaytonaInvalidArgumentError('requestTimeoutMs must be a non-negative finite number of milliseconds');
                }
            ...
            this.secret = new Secret_1.SecretService(new api_client_1.SecretApi(configuration, '', axiosInstance));
            this.warmPool = new WarmPool_1.WarmPoolService(new api_client_1.WarmPoolsApi(configuration, '', axiosInstance));
            ...
            const useDeprecatedPolling = config?.useDeprecatedPolling ?? envReader()?.get('DAYTONA_USE_DEPRECATED_POLLING') === 'true';
            if (!useDeprecatedPolling) {
                this.eventDispatcher = new EventDispatcher_1.EventDispatcher(this.apiUrl, this.apiKey || this.jwtToken, this.organizationId, sdkLabel, packageJson.version);
                this.eventDispatcher.ensureConnected();
            }
            this.eventSubscriptionManager = new EventSubscriptionManager_1.EventSubscriptionManager(this.eventDispatcher ?? null);
            const env = envReader();
        ...
        async create(params, options = { timeout: 60 }) {
            ...
            if (params.language) {
                ...
                if (!validLanguages.includes(params.language)) {
                    throw new DaytonaError_1.DaytonaInvalidArgumentError(`Invalid ${exports.CODE_TOOLBOX_LANGUAGE_LABEL}: ${params.language}. Supported languages: ${validLanguages.join(', ')}`);
                }
                labels[exports.CODE_TOOLBOX_LANGUAGE_LABEL] = params.language;
            }
            if (options.timeout < 0) {
                throw new DaytonaError_1.DaytonaInvalidArgumentError('Timeout must be a non-negative number');
            }
            if (params.autoStopInterval !== undefined &&
                (!Number.isInteger(params.autoStopInterval) || params.autoStopInterval < 0)) {
                throw new DaytonaError_1.DaytonaInvalidArgumentError('autoStopInterval must be a non-negative integer');
            }
            if (params.autoPauseInterval !== undefined &&
                (!Number.isInteger(params.autoPauseInterval) || params.autoPauseInterval < 0)) {
                throw new DaytonaError_1.DaytonaInvalidArgumentError('autoPauseInterval must be a non-negative integer');
            }
            if (params.autoStopInterval && params.autoPauseInterval) {
                throw new DaytonaError_1.DaytonaInvalidArgumentError('autoStopInterval and autoPauseInterval are mutually exclusive. Set at most one of them to a non-zero value');
            }
            if (params.autoPauseInterval && (params.ephemeral || params.autoDeleteInterval === 0)) {
                throw new DaytonaError_1.DaytonaInvalidArgumentError('Ephemeral sandboxes cannot have auto-pause enabled. Set autoPauseInterval to 0');
            }
            if (params.ephemeral) {
            ...
            if (params.autoArchiveInterval !== undefined &&
                ...
                throw new DaytonaError_1.DaytonaInvalidArgumentError('autoArchiveInterval must be a non-negative integer');
            }
            if (params.ttlMinutes !== undefined && (!Number.isInteger(params.ttlMinutes) || params.ttlMinutes < 0)) {
                throw new DaytonaError_1.DaytonaInvalidArgumentError('ttlMinutes must be a non-negative integer');
            }
            try {
                let buildInfo;
                let snapshot;
                ...
                const sandbox = new Sandbox_1.Sandbox(await this.ensureToolboxProxyUrl(sandboxInstance), new api_client_1.Configuration(structuredClone(this.clientConfig)), Daytona.createAxiosInstance(this.requestTimeoutMs), this.sandboxApi, this.getAnalyticsApiUrl, this.eventSubscriptionManager, this.requestTimeoutMs);
                ...
                if (error instanceof DaytonaError_1.DaytonaTimeoutError) {
                    ...
                    throw new DaytonaError_1.DaytonaTimeoutError(errMsg, error.statusCode, error.headers, error.code, error.source);
                }
                throw error;
            }
        }
        ...
        async get(sandboxIdOrName) {
            ...
            return new Sandbox_1.Sandbox(sandboxInstance, structuredClone(this.clientConfig), Daytona.createAxiosInstance(this.requestTimeoutMs), this.sandboxApi, this.getAnalyticsApiUrl, this.eventSubscriptionManager, this.requestTimeoutMs);
        }
        /**
        ...
        list(query) {
            ...
            async function* generator() {
                ...
                while (true) {
                    ...
                    let response;
                    try {
                        response = await api_1.context.with(api_1.trace.setSpan(api_1.context.active(), span), () => sandboxApi.listSandboxes(undefined, cursor, query?.limit, query?.id, query?.name, query?.labels ? JSON.stringify(query.labels) : undefined, undefined, undefined, query?.states, query?.snapshots, query?.targets, undefined, query?.minCpu, query?.maxCpu, query?.minMemoryGib, query?.maxMemoryGib, query?.minDiskGib, query?.maxDiskGib, query?.isPublic, query?.isRecoverable, query?.createdAtAfter, query?.createdAtBefore, query?.lastActivityAfter, query?.lastActivityBefore, query?.autoDestroyAtAfter, query?.autoDestroyAtBefore, query?.sort, query?.order));
                    ...
                    for (const sandbox of response.data.items) {
                        ...
                        yield new Sandbox_1.Sandbox(hydratedSandbox, structuredClone(clientConfig), Daytona.createAxiosInstance(requestTimeoutMs), sandboxApi, getAnalyticsApiUrl, eventSubscriptionManager, requestTimeoutMs);
                    }
        ...
        async ensureToolboxProxyUrl(sandboxDto) {
         ...
         */
        async fork(sandbox, params, timeout = 60) {
         ...
         */
        async delete(sandbox, timeout = 60, wait = false) {
            await sandbox.delete(timeout, wait);
        }
        /**
         * @hidden
         */
        static createAxiosInstance(requestTimeoutMs) {
