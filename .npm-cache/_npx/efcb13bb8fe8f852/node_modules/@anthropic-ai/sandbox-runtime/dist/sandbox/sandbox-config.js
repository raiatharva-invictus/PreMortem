...
import { z } from 'zod';
import { isInjectHostCoveredByAllowedDomains, splitDomainPatternPort, stripDomainPatternPort, } from './domain-pattern.js';
/**
...
const DOMAIN_PATTERN_MESSAGE = 'Invalid domain pattern. Must be a valid domain (e.g., "example.com"), a wildcard (e.g., "*.example.com"), or a bracketed IPv6 literal (e.g., "[::1]", "[2001:db8::1]:443"). Overly broad patterns like "*.com" or "*" are not allowed for security reasons.';
/**
 ...
 */
const deniedDomainPatternSchema = z.string().refine(val => {
    if (!hasValidIpv6Bracketing(val))
        return false;
...
const envVarNameSchema = z
    .string()
    .regex(/^[A-Za-z_][A-Za-z0-9_]*$/, 'Environment variable name must start with a letter or underscore and contain only letters, digits, and underscores');
/**
...
const extractPatternSchema = z.string().superRefine((val, ctx) => {
    ...
    try {
        ...
        ctx.addIssue({
            ...
            message: `extract is not a valid regular expression: ${err.message}`,
        });
        return;
    }
    ...
    if (groupCount < 1) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: 'extract must contain at least one capturing group — group 1 is ' +
...
export const CredentialFileConfigSchema = z.object({
    path: filesystemPathSchema.describe('Path to a credential file or directory. Supports the same path forms as ' +
    ...
    extract: extractPatternSchema
        .optional()
        .describe('Optional regex for structured masking. Applied globally; capture ' +
    ...
    onExtractNoMatch: z
        ...
        .optional()
        .describe('What to do when extract matches nothing (or, with decode, no ' +
    ...
    decode: z
        .enum(['jwt'])
        .optional()
        .describe('Optional encoded-credential format. "jwt": decodes recognized JWTs ' +
    ...
    maskDuplicates: z
        .boolean()
        .optional()
        .describe('If true, verbatim occurrences of each captured credential value ' +
    ...
    injectHosts: z
        .array(domainPatternSchema)
        .optional()
        .describe('Optional narrowing of where the proxy substitutes this credential. ' +
...
export const CredentialEnvVarConfigSchema = z.object({
    ...
    mode: credentialModeSchema.describe('Access mode for this environment variable'),
    extract: extractPatternSchema
        .optional()
        .describe('Optional regex for structured masking. Applied globally; capture ' +
    ...
    onExtractNoMatch: z
        .enum(['warn', 'deny', 'error'])
        .optional()
        .describe('What to do when extract matches nothing: "warn" (default — stderr ' +
    ...
    decode: z
        .enum(['jwt'])
        .optional()
        .describe('Optional encoded-credential format. "jwt": the variable\'s whole ' +
    ...
    maskClaims: z
        .array(z.string().min(1))
        .optional()
        .describe('Names of top-level payload claims to mask inside the decoded ' +
        'value, instead of replacing the whole token. Each named claim ' +
        'present with a string value gets its own sentinel; all other ' +
        'claims are preserved so claim-reading clients keep working. ' +
...
export const AwsPairConfigSchema = z
    .object({
    accessKeyIdVar: envVarNameSchema.describe('Name of the masked env var holding the AWS access key id.'),
    secretAccessKeyVar: envVarNameSchema.describe('Name of the masked env var holding the AWS secret access key.'),
    sessionTokenVar: envVarNameSchema
        .optional()
        .describe('Optional name of the masked env var holding the AWS session ' +
...
export const Sigv4ConfigSchema = z
    ...
    streaming: sigv4PolicySchema
        .optional()
        .describe('Policy for aws-chunked streaming uploads (x-amz-content-sha256: ' +
    ...
    presigned: sigv4PolicySchema
        .optional()
        .describe('Policy for presigned URLs (X-Amz-Algorithm/X-Amz-Signature in ' +
        'the query, no Authorization header): the signature lives in ' +
        'the URL itself. Default "deny".'),
    sigv4a: sigv4PolicySchema
        .optional()
        .describe('Policy for SigV4A (AWS4-ECDSA-P256-SHA256) asymmetric ' +
...
export const CredentialsConfigSchema = z
    ...
    allowPlaintextInject: z
        .boolean()
        .optional()
        .describe('Allow sentinel→real substitution on the plain-HTTP proxy path. ' +
    ...
    awsPairs: z
        ...
        .optional()
        .describe('Explicit groupings of masked env vars into AWS credential pairs ' +
    ...
    sigv4: Sigv4ConfigSchema.optional().describe('Policies for AWS SigV4 request shapes the proxy cannot re-sign ' +
...
export const NetworkConfigSchema = z.object({
    ...
    deniedDomainReasons: z
        ...
        .optional()
        .describe('Optional model-facing reason keyed by the exact deniedDomains entry it explains (e.g., {"github.com:22": "SSH pushes to GitHub are blocked; use an https remote"}). ' +
    ...
    tlsTerminate: z
        ...
        excludeDomains: z
            .array(domainPatternSchema)
            .optional()
            .describe('Domain patterns (same syntax as allowedDomains) whose HTTPS ' +
        ...
        extraCaCertPaths: z
            .array(z.string().min(1))
            .optional()
            .describe('Paths to PEM CA certificate files appended to the trust bundle ' +
...
export const FilesystemConfigSchema = z.object({
    disabled: z
        .boolean()
        .optional()
        .describe('Disable all filesystem policy enforcement. When true, no read or write rules are emitted: ' +
...
export const GitConfigSchema = z.object({
    safeDirectories: z
        .array(z.string())
        .default([])
        .describe('Directories git should treat as safe (`safe.directory`) even though ' +
...
export const SrtWinConfigSchema = z.object({
    path: z
        .string()
        .min(1)
        .describe('Path to the srt-win binary. Pass the exported ' +
...
export const WindowsConfigSchema = z.object({
    sandboxUser: z
        ...
        .optional()
        .describe('Name for the dedicated sandbox user account that `srt-win install` ' +
    ...
    proxyPortRange: z
        ...
        .optional()
        .describe('Inclusive [low, high] port range the JS mux proxy listeners bind ' +
    ...
    srtWin: SrtWinConfigSchema.optional().describe('How to locate/invoke the srt-win helper binary. `srtWin.path` is ' +
...
export const SandboxRuntimeConfigSchema = z
    ...
    credentials: CredentialsConfigSchema.optional().describe('Credential handling configuration. Only the explicitly declared files ' +
    ...
    windows: WindowsConfigSchema.optional().describe('Windows-specific settings (WFP sublayer, proxy port range).'),
    git: GitConfigSchema.optional().describe('Git configuration to inject via GIT_CONFIG_* env vars ' +
        ...
        for (const [i, host] of hosts.entries()) {
            if (!isInjectHostCoveredByAllowedDomains(host, allowedHosts)) {
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
                    path: [...path, i],
                    message: `injectHosts entry "${host}" is not reachable via ` +
    ...
    let hasMasked = false;
    const checkMaskedEntry = (entry, path) => {
        ...
        if (exclude?.length) {
            if (entry.injectHosts) {
                for (const [i, host] of entry.injectHosts.entries()) {
                    if (isInjectHostCoveredByAllowedDomains(host, exclude)) {
                        ctx.addIssue({
                            ...
                            message: `injectHosts entry "${host}" is entirely covered by ` +
                ...
                allowedHosts.every(p => isInjectHostCoveredByAllowedDomains(p, exclude))) {
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
                    path,
                    message: `This masked credential has no injectHosts, so it defaults ` +
        ...
        if (entry.injectHosts !== undefined && entry.injectHosts.length === 0) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                path,
                message: `injectHosts is explicitly empty — the credential would be ` +
    ...
    for (const [idx, v] of (creds.envVars ?? []).entries()) {
        ...
        if (v.maskClaims !== undefined && v.decode === undefined) {
            ctx.addIssue({
                ...
                message: `maskClaims requires decode — it names claims inside the ` +
        ...
        if (v.maskClaims !== undefined && v.maskClaims.length === 0) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                path: ['credentials', 'envVars', idx, 'maskClaims'],
                message: `maskClaims is explicitly empty — no claim would ever be ` +
    ...
    for (const [idx, f] of (creds.files ?? []).entries()) {
        ...
        if (f.mode === 'mask' && f.path.endsWith('/')) {
            ctx.addIssue({
                ...
                message: `Credential mode "mask" applies to a single file, not a ` +
        ...
        for (const [field, name] of vars) {
            ...
            if (seenPairVars.has(name)) {
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
                    path,
                    message: `"${name}" appears in more than one awsPairs slot (within ` +
            ...
            if (entry === undefined || entry.mode !== 'mask') {
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
                    path,
                    message: `"${name}" does not reference a credentials.envVars entry ` +
                ...
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
                    path,
                    message: `"${name}" uses extract/decode, but an AWS pair member ` +
    ...
    if (hasMasked &&
        ...
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: ['credentials'],
            message: 'Credential masking requires network.tlsTerminate so substitution ' +
