...
async function main() {
    ...
    program
        ...
        .action(async (o) => {
        const { installWindowsSandbox, resolveSrtWin, VENDORED_SRT_WIN_EXE } = await import('./sandbox/windows-sandbox-utils.js');
        const range = typeof o.proxyPortRange === 'string'
            ? o.proxyPortRange.split('-').map(Number)
            : undefined;
        try {
    ...
    program
        ...
        .option('--sublayer-guid <guid>', 'WFP sublayer GUID')
        .action(async (o) => {
        const { uninstallWindowsSandbox, resolveSrtWin, VENDORED_SRT_WIN_EXE } = await import('./sandbox/windows-sandbox-utils.js');
        try {
    ...
    program
        ...
        try {
            ...
            if (!runtimeConfig) {
                ...
                if (options.settings) {
                    console.error(`Error: Could not load settings from ${configPath} (missing, unreadable, or invalid). ` +
            ...
            if (process.platform === 'win32' &&
                ...
                const { VENDORED_SRT_WIN_EXE } = await import('./sandbox/windows-sandbox-utils.js');
                runtimeConfig = {
                    ...runtimeConfig,
                    windows: {
