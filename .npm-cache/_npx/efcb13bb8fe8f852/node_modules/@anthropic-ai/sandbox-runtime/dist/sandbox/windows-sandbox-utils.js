...
import { generateProxyEnvVars, encodeSandboxedCommand, buildGitConfigEnv, normalizePathForSandbox, containsGlobCharsWin, expandGlobPattern, isUncPath, } from './sandbox-utils.js';
...
export { containsGlobCharsWin, stripExtendedPathPrefix, buildGitConfigEnv, isUncPath, } from './sandbox-utils.js';
...
export function parseWindowsBinShell(raw) {
    ...
    if (typeof raw === 'object') {
        if (!path.win32.isAbsolute(raw.exe)) {
            throw new WindowsSandboxError('bin_shell_invalid', `binShell.exe must be an absolute path ` +
                `(got ${JSON.stringify(raw.exe)})`);
        }
        if (!Array.isArray(raw.args)) {
            throw new WindowsSandboxError('bin_shell_invalid', `binShell.args must be an array (got ${JSON.stringify(raw.args)})`);
        }
        return raw;
    }
    ...
    if (!isAbs && raw !== rawBase) {
        throw new WindowsSandboxError('bin_shell_invalid', `binShell string must be a bare token or an absolute path ` +
    ...
    switch (base) {
        ...
        case 'sh.exe':
            ...
            if (!isAbs) {
                throw new WindowsSandboxError('bin_shell_invalid', `binShell bash path must be absolute ` +
        ...
        default:
            throw new WindowsSandboxError('bin_shell_invalid', `unrecognised binShell ${JSON.stringify(raw)}: expected ` +
 ...
 */
export const VENDORED_SRT_WIN_EXE = path.join(repoRoot(), 'vendor', 'srt-win', nodeArchToDir[process.arch] ?? process.arch, 'srt-win.exe');
/**
...
export function getSrtWinPath() {
    ...
    throw new WindowsSandboxError('srt_win_not_found', `srt-win.exe not found. Set windows.srtWin.path or build with ` +
...
export function resolveSrtWin(cfg) {
    if (cfg?.path === undefined) {
        throw new WindowsSandboxError('srt_win_not_found', `no srt-win path configured; set windows.srtWin.path (e.g. to the ` +
            `exported VENDORED_SRT_WIN_EXE constant for the packaged binary)`);
    }
    if (!fs.existsSync(cfg.path)) {
        throw new WindowsSandboxError('srt_win_not_found', `windows.srtWin.path is set to '${cfg.path}' but the file does ` +
            `not exist`);
    }
...
function runSrtWin(args, opts = {}) {
    ...
    if (r.error) {
        ...
        if (r.error.code === 'ETIMEDOUT') {
            throw new WindowsSandboxError('srt_win_timeout', `srt-win ${args.join(' ')} timed out after ` +
                `${opts.timeoutMs ?? 15000}ms` +
                (r.signal ? ` (killed by ${r.signal})` : ''), args[0]);
        }
        throw new WindowsSandboxError('spawn_failed', `srt-win ${args[0]}: spawn failed: ${r.error.message}`, args[0]);
    }
    return {
        status: r.status,
        signal: r.signal,
 ...
 */
function runSrtWinAsync(args, opts = {}) {
    const { exe, prependArgs } = opts.srtWin ?? resolveSrtWin();
    ...
    return new Promise((resolve, reject) => {
        ...
        child.once('error', e => {
            clearTimeout(timer);
            reject(new WindowsSandboxError('spawn_failed', `srt-win ${args[0]}: spawn failed: ${e.message}`, args[0]));
        });
        child.once('close', (status, signal) => {
            clearTimeout(timer);
            if (timedOut) {
                reject(new WindowsSandboxError('srt_win_timeout', `srt-win ${args.join(' ')} timed out after ${timeoutMs}ms` +
                    (signal ? ` (killed by ${signal})` : ''), args[0]));
                return;
            }
...
function parseSrtWinJson(args, r) {
    if (r.status !== 0) {
        throw new WindowsSandboxError('srt_win_nonzero', `srt-win ${args.join(' ')} exited ${r.status}: ${r.stderr || r.stdout}`, args[0]);
    }
    try {
        return JSON.parse(r.stdout);
    }
    catch (e) {
        throw new WindowsSandboxError('srt_win_bad_json', `srt-win ${args.join(' ')}: unparseable JSON output ` +
            `${JSON.stringify(r.stdout)}: ${e.message}`, args[0]);
    }
}
function runSrtWinJson(args, opts) {
    return parseSrtWinJson(args, runSrtWin(args, opts));
}
async function runSrtWinJsonAsync(args, opts) {
...
function wfpStatusArgs(sublayerGuid) {
    const args = ['wfp', 'status'];
    if (sublayerGuid)
        args.push('--sublayer-guid', sublayerGuid);
    return args;
}
function mapWfpStatus(raw) {
    return {
        state: raw.state,
        filters: raw.filters,
 ...
 */
export function checkWindowsSandboxStatus(opts = {}) {
    const args = ['status'];
...
export async function checkWindowsSandboxStatusAsync(opts = {}) {
    const args = ['status'];
    if (opts.sublayerGuid)
        args.push('--sublayer-guid', opts.sublayerGuid);
    const raw = await runSrtWinJsonAsync(args, { srtWin: opts.srtWin });
    return { user: mapUserStatus(raw.user), wfp: mapWfpStatus(raw.wfp) };
}
/**
...
export function getWindowsWfpStatus(opts = {}) {
    return mapWfpStatus(runSrtWinJson(wfpStatusArgs(opts.sublayerGuid), {
        srtWin: opts.srtWin,
    }));
}
/** Async twin of {@link getWindowsWfpStatus}. */
export async function getWindowsWfpStatusAsync(opts = {}) {
    return mapWfpStatus(await runSrtWinJsonAsync(wfpStatusArgs(opts.sublayerGuid), {
        srtWin: opts.srtWin,
    }));
}
/**
...
export async function verifyWindowsWfpEgress(opts = {}) {
    ...
    if (!target) {
        ...
        if (!target) {
            throw new WindowsSandboxError('wfp_verify_bind_failed', `verifyWindowsWfpEgress: could not bind a loopback ` +
    ...
    try {
        ...
        try {
            ...
            throw new WindowsSandboxError('wfp_verify_unparseable', `WFP egress fence could not be verified — \`srt-win wfp ` +
        ...
        if (r.status === 3) {
            throw new WindowsSandboxError('wfp_fence_inactive', `WFP egress fence is not active — direct outbound from the ` +
                `sandbox user to ${raw.target} succeeded. Re-run ` +
                `\`srt-win install\` (one UAC prompt). (${r.stderr})`);
        }
        if (r.status !== 0) {
            throw new WindowsSandboxError('wfp_verify_inconclusive', `WFP egress fence could not be verified — probe to ` +
...
export function getWindowsSandboxUserStatus(opts = {}) {
    return mapUserStatus(runSrtWinJson(['user', 'status'], { srtWin: opts.srtWin }));
}
/** Async twin of {@link getWindowsSandboxUserStatus}. */
export async function getWindowsSandboxUserStatusAsync(opts = {}) {
    return mapUserStatus(await runSrtWinJsonAsync(['user', 'status'], {
        srtWin: opts.srtWin,
    }));
}
/**
...
export async function windowsTrustCaAsync(caCertPath, opts = {}) {
...
function checkTrustCaResult(caCertPath, r) {
    logForDebugging(`[Sandbox Windows] user trust-ca exit=${r.status}: ${r.stderr || r.stdout}`);
    if (r.status !== 0) {
        throw new WindowsSandboxError('trust_ca_failed', `srt-win user trust-ca '${caCertPath}' failed (exit ` +
 ...
 * custody.
 */
export function windowsStateDir() {
 ...
 *   be created.
 */
export async function ensurePersistentWindowsCa(opts = {}) {
    ...
    if (!status.provisioned || !status.credPresent) {
        throw new Error(`ensurePersistentWindowsCa: sandbox user is not provisioned ` +
    ...
    if (stateDir && !fs.existsSync(stateDir)) {
        throw new Error(`ensurePersistentWindowsCa: state directory ${stateDir} does not ` +
    ...
    const finish = async (certPem, keyPem, thumbprint, generated) => {
        ...
        logForDebugging(`[Sandbox Windows] persistent CA ` +
    ...
    const readValid = () => {
        ...
        try {
            ...
            logForDebugging(`[Sandbox Windows] persistent CA ca.json unreadable ` +
                `(${err.message})`, { level: 'warn' });
            return undefined;
        }
    ...
    if (!opts.force) {
        ...
        if (on) {
            logForDebugging(`[Sandbox Windows] persistent CA expiring ` +
                `${on.notAfter.toISOString()}; regenerating`, { level: 'warn' });
        }
    }
    ...
    try {
        ...
        for (const f of fs.readdirSync(dir)) {
            if (!f.includes('.tmp.') && !f.startsWith('.trust.'))
                continue;
    ...
    atomicWrite(jsonPath, JSON.stringify({ certPem: gen.certPem, keyPem: gen.keyPem }));
    const on = readValid();
 ...
 */
function installTimeoutMs(opts) {
    return opts.timeoutMs ?? 120000;
}
 ...
 */
function remapInstallTimeout(e) {
    if (e instanceof WindowsSandboxError && e.code === 'srt_win_timeout') {
        throw new WindowsSandboxError('install_timeout', `srt-win install timed out — the UAC prompt may still be open. ` +
...
function throwOnInstallFailure(r) {
    ...
    switch (r.status) {
        case 12:
            throw new WindowsSandboxError('install_wfp_failed', `srt-win install: WFP filter install failed: ${out}`);
        case 14:
            throw new WindowsSandboxError('install_user_failed', `srt-win install: sandbox user provisioning failed: ${out}`);
        case 13:
            throw new WindowsSandboxError('install_config_conflict', `srt-win install: filters already exist under this sublayer with ` +
                `a different port range or sandbox-user name. Pass ` +
                `{force: true} to replace, or pick a different sublayerGuid. ` +
                `Output: ${out}`);
        default:
            throw new WindowsSandboxError('install_failed', `srt-win install failed (exit ${r.status}): ${out}`);
    }
}
/**
 ...
 * keep firing.
 */
export async function installWindowsSandboxAsync(opts = {}) {
    const srtWin = opts.srtWin ?? resolveSrtWin();
    let r;
    try {
...
export function uninstallWindowsSandbox(opts = {}) {
    ...
    if (r.status !== 0) {
        throw new WindowsSandboxError('uninstall_failed', `srt-win uninstall failed (exit ${r.status}): ${r.stderr || r.stdout}`);
    }
    return {};
}
/**
 ...
 */
export function expandWindowsFsPaths(patterns, opts) {
    const out = new Set();
 ...
 */
export function parseWindowsSandboxError(stderr) {
    for (const line of stderr.split(/\r?\n/)) {
        ...
        try {
            ...
            if (j.code === 'mapped_drive_cwd') {
                ...
                return Object.assign(new WindowsSandboxError('mapped_drive_cwd', j.message ?? 'mapped/network-drive working directory', 'exec'), { drive: j.drive });
            }
        }
        catch {
            // not our line
...
export function stampWindowsAcl(opts) {
    ...
    if (r.status !== 0) {
        throw new WindowsSandboxError('acl_stamp_failed', `srt-win acl stamp exited ${r.status} ` +
...
export function grantWindowsAcl(opts) {
    ...
    if (r.status !== 0) {
        throw new WindowsSandboxError('acl_grant_failed', `srt-win acl grant exited ${r.status}: ${r.stderr || r.stdout}`);
    }
}
/**
...
export function wrapCommandWithSandboxWindows(p) {
    ...
    const generated = envListToObject(generateProxyEnvVars(p.httpProxyPort, p.socksProxyPort, p.caCertPath?.replace(/\\/g, '/'), p.proxyAuthToken, undefined, encodeSandboxedCommand(p.commandId ?? p.command)));
    ...
    if (cmdlineEstimate > 30000) {
        throw new WindowsSandboxError('argv_too_long', `Windows sandbox argv is ~${cmdlineEstimate} chars ` +
 ...
 */
function interpretDependencyProbes(sublayerGuid, user, getWfp) {
    const errors = [];
    const warnings = [];
    // 2. Sandbox user provisioned + credential readable.
    if (user.status === 'rejected') {
        errors.push(`srt-win user status failed: ${user.reason.message}`);
    ...
    if (wfp.status === 'rejected') {
        errors.push(`srt-win wfp status failed: ${wfp.reason.message}`);
        return { errors, warnings };
    }
    const ws = wfp.value;
...
export function checkWindowsDependencies(opts = {}) {
    ...
    return interpretDependencyProbes(opts.sublayerGuid, settle(() => getWindowsSandboxUserStatus({ srtWin })), 
    // Lazy: spawned only if user-status succeeded (short-circuit).
    () => settle(() => getWindowsWfpStatus({ sublayerGuid: opts.sublayerGuid, srtWin })));
}
/**
 ...
 */
export async function checkWindowsDependenciesAsync(opts = {}) {
    let srtWin;
    try {
        srtWin = opts.srtWin ?? resolveSrtWin();
    }
