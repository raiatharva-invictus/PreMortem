...
import { SUB_AGENT_IDENTITY } from "../capabilities/builtins/DynamicSubAgents.mjs";
import { AgentHarnessError, InvalidAgentSendInputError } from "../errors.mjs";
import {
  EventType,
  newEventId
} from "../events/schema.mjs";
import { InstructionBuilder, ROOT_AGENT_IDENTITY } from "../InstructionBuilder.mjs";
import "../llm/LLMTypes.mjs";
import { toOpenAIResponseFormat } from "../llm/responseFormat.mjs";
import { toOpenAIChatMessage } from "../llm/toOpenAIChatMessage.mjs";
import { estimateTokensForString } from "../llm/usage.mjs";
import { convertMCPServersToTools } from "../mcp/convertMCPServers.mjs";
import { executeToolCalls } from "../mcp/executeToolCalls.mjs";
import { describeUnknownError, extractErrorLogFields } from "../util/errorLogFields.mjs";
import {
  InternalEventType
} from "./AgentThread.types.mjs";
import {
  currentContextUsageFromCompletion,
  getEmptyCurrentContextUsage,
  mergeCurrentContextUsage
} from "./contextUsage.mjs";
import {
  ...
  toEnrichedToolCall,
  toToolCallInfo
} from "./contextUtils.mjs";
import { DeferredTool } from "./DeferredTool.mjs";
import { createEmptyAgentThreadMetrics, updateMetricsFromUsage } from "./metrics.mjs";
import { getClosableOpenToolCallIds, OpenToolCallCloser } from "./OpenToolCallCloser.mjs";
import { isEmptyMessageContent, processAgentUserInput } from "./UserInputMessage.mjs";
...
function lastAssistantInContext(context) {
  return context.findLast(
    (msg) => isLLMContextMessage(msg) && msg.role === "assistant"
  );
}
...
var AgentThread = class {
  ...
  tracing;
  logger;
  metrics = createEmptyAgentThreadMetrics();
  ...
  *appendToContext(opts) {
    ...
    const newCurrentContextUsage = currentContextUsage ?? mergeCurrentContextUsage(this.currentContextUsage, estimateTokensForContextMessages(context));
    const event = {
  ...
  *overwriteContext(payload) {
  ...
  transformToLLMRequest(tools) {
    ...
    const requestBody = {
      messages,
      stream: true,
      ...this.definition.responseFormat !== void 0 ? { response_format: toOpenAIResponseFormat(this.definition.responseFormat) } : {}
    };
...
//# sourceMappingURL=AgentThread.mjs.map