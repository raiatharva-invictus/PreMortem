    ...
    return {
        ...
        listSandboxes: async (xDaytonaOrganizationID, cursor, limit, id, name, labels, includeErroredDeleted, includeWarm, states, snapshots, regionIds, sandboxClasses, minCpu, maxCpu, minMemoryGiB, maxMemoryGiB, minDiskGiB, maxDiskGiB, isPublic, isRecoverable, createdAtAfter, createdAtBefore, lastEventAfter, lastEventBefore, autoDestroyAtAfter, autoDestroyAtBefore, sort, order, options = {}) => {
            ...
            if (autoDestroyAtAfter !== undefined) {
                localVarQueryParameter['autoDestroyAtAfter'] = (autoDestroyAtAfter instanceof Date) ?
                    autoDestroyAtAfter.toISOString() :
                    autoDestroyAtAfter;
            }
            if (autoDestroyAtBefore !== undefined) {
                localVarQueryParameter['autoDestroyAtBefore'] = (autoDestroyAtBefore instanceof Date) ?
                    autoDestroyAtBefore.toISOString() :
                    autoDestroyAtBefore;
            }
            if (sort !== undefined) {
        ...
        setAutoPauseInterval: async (sandboxIdOrName, interval, xDaytonaOrganizationID, options = {}) => {
            ...
            const localVarPath = `/sandbox/{sandboxIdOrName}/autopause/{interval}`
                .replace(`{${"sandboxIdOrName"}}`, encodeURIComponent(String(sandboxIdOrName)))
                .replace(`{${"interval"}}`, encodeURIComponent(String(interval)));
            // use dummy base URL string because the URL constructor only accepts absolute URLs.
            const localVarUrlObj = new URL(localVarPath, common_1.DUMMY_BASE_URL);
