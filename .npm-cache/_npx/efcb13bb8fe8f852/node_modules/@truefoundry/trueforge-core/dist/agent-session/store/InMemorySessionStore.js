...
var import_contextUsage = require("../../core/runtime/contextUsage.js");
var import_assertCreateTurnThreadDelta = require("./assertCreateTurnThreadDelta.js");
var import_OffsetPageToken = require("./OffsetPageToken.js");
var import_SessionEventPageToken = require("./SessionEventPageToken.js");
var import_SessionListPageToken = require("./SessionListPageToken.js");
var import_SessionStoreErrors = require("./SessionStoreErrors.js");
...
function turnKey({ session_id, turn_id }) {
  return `${session_id}:${turn_id}`;
}
function compareSessionId(a, b) {
  if (a < b) {
    return -1;
  }
  if (a > b) {
...
function newThreadSnapshot(thread) {
  return {
    ...
    context: [],
    current_context_usage: (0, import_contextUsage.getEmptyCurrentContextUsage)(),
    completion: null
  };
}
function applyContextAppends(threads, appends) {
  for (const append of appends) {
    const thread = threads[append.thread_id];
    if (!thread) {
      throw new import_SessionStoreErrors.SessionStoreInvariantError(`new_context_appends references unknown thread ${append.thread_id}`);
    }
...
function buildSnapshotFromDelta(input) {
  const threads = input.previousSnapshot ? deepCopy(input.previousSnapshot.threads) : {};
  (0, import_assertCreateTurnThreadDelta.assertCreateTurnThreadDelta)({
  ...
  for (const capability of input.capability_states) {
    ...
    if (thread === void 0) {
      throw new import_SessionStoreErrors.SessionStoreInvariantError(`capability_states references unknown thread ${capability.thread_id}`);
    }
...
function paginate(items, limit, pageToken) {
  const offset = (0, import_OffsetPageToken.decodeOffsetPageToken)(pageToken);
  const slice = items.slice(offset, offset + limit);
  return {
    data: slice,
    pagination: {
      limit,
      ...offset + limit < items.length ? { next_page_token: (0, import_OffsetPageToken.encodeOffsetPageToken)(offset + limit) } : {},
      ...offset > 0 ? { previous_page_token: (0, import_OffsetPageToken.encodeOffsetPageToken)(Math.max(0, offset - limit)) } : {}
    }
  };
}
var InMemorySessionStore = class {
  ...
  async updateSession(input) {
    ...
    if (stored?.record.tenant_id !== input.tenant_id) {
      throw new import_SessionStoreErrors.SessionNotFoundError(input.session_id);
    }
    if (input.agent !== void 0) {
      if (stored.record.agent.type === "reference") {
        throw new import_SessionStoreErrors.SessionStoreInvariantError(`Session ${input.session_id} is named; agent cannot be updated`);
      }
  ...
  async listSessions(input) {
    ...
    for (const stored of this.sessions.values()) {
      ...
      if (input.created_by !== void 0 && stored.record.created_by !== input.created_by) {
        continue;
      }
      ...
      if (input.end_timestamp !== void 0 && createdAt > input.end_timestamp.getTime()) {
        continue;
      }
    ...
    records.sort((a, b) => {
      ...
      return input.order === "asc" ? compareSessionId(a.session_id, b.session_id) : compareSessionId(b.session_id, a.session_id);
    });
    let filtered = records;
    const cursor = (0, import_SessionListPageToken.decodeSessionListPageToken)(input.page_token);
    if (cursor) {
    ...
    const page = (0, import_SessionListPageToken.paginateSessionListRows)(filtered, input.limit, (row) => row.updated_at.toISOString());
  ...
  async createTurn(input) {
    ...
    const stored = this.sessions.get(sKey);
    if (!stored) {
      throw new import_SessionStoreErrors.SessionNotFoundError(input.turn.session_id);
    }
    ...
    if (previousTurnId !== null) {
      ...
      if (prev !== void 0) {
        if (prev.state.status === "running") {
          throw new import_SessionStoreErrors.PreviousTurnRunningError(previousTurnId);
        }
    ...
    if (this.turns.has(tKey)) {
      throw new import_SessionStoreErrors.TurnAlreadyExistsError(input.turn.turn_id);
    }
  ...
  async freezeAndGetTurn(input) {
    ...
    if (turn.state.status === "running") {
      ...
      turn.updated_at = /* @__PURE__ */ new Date();
  ...
  async getTurn(input) {
    ...
    return turn ? deepCopy(turn) : void 0;
  }
  async listTurns(input) {
    ...
    const records = stored.turnIds.map((id) => {
      ...
      if (!turn) {
        throw new import_SessionStoreErrors.TurnNotFoundError(id);
      }
  ...
  async updateTurnState(input) {
    ...
    const turn = this.requireTurn(input.session_id, input.turn_id);
    if (turn.state.status !== "running") {
      throw new import_SessionStoreErrors.TurnNotRunningError(input.turn_id, turn.state);
    }
  ...
  async appendToEvents(input) {
    ...
    const list = this.events.get(tKey);
    if (!list) {
      throw new import_SessionStoreErrors.TurnNotFoundError(input.turn_id);
    }
  ...
  requireTurn(sessionId, turnId) {
    ...
    if (!turn) {
      throw new import_SessionStoreErrors.TurnNotFoundError(turnId);
    }
    return turn;
  }
  requireRunningTurn(sessionId, turnId) {
    const turn = this.requireTurn(sessionId, turnId);
    if (turn.state.status !== "running") {
      throw new import_SessionStoreErrors.TurnNotRunningError(turnId, turn.state);
    }
    return turn;
  }
  ...
  async appendToThreadContext(input) {
    ...
    if (!thread) {
      throw new import_SessionStoreErrors.SessionStoreInvariantError(`Thread not found: ${input.thread_id}`);
    }
  ...
  async overwriteThreadContext(input) {
    ...
    if (!thread) {
      throw new import_SessionStoreErrors.SessionStoreInvariantError(`Thread not found: ${threadId}`);
    }
  ...
  async listTurnEvents(input) {
    ...
    const ordered = [...list].sort(
      (a, b) => input.order === "desc" ? b.id.localeCompare(a.id) : a.id.localeCompare(b.id)
    );
  ...
  async listSessionEvents(input) {
    ...
    const decodedCursor = input.page_token === void 0 ? void 0 : (0, import_SessionEventPageToken.decodeSessionEventPageToken)(input.page_token);
    ...
    if (!anchor) {
      throw new import_SessionStoreErrors.TurnNotFoundError(cursor.last_turn_id);
    }
    ...
    const page = (0, import_SessionEventPageToken.paginateSessionEventRows)(
   ...
   */
  resolveAncestorChain(sessionId, anchor) {
