...
import { AwsPairRegistry, createSigv4Planner, registerAwsPairs, } from './credential-aws-pairs.js';
import { certThumbprint, createMitmCA, CRL_PATH, disposeMitmCA, } from './mitm-ca.js';
...
import { checkWindowsDependencies, checkWindowsDependenciesAsync, wrapCommandWithSandboxWindows, parseWindowsBinShell, expandWindowsFsPaths, stampWindowsAcl, restoreWindowsAcl, grantWindowsAcl, revokeWindowsAcl, getWindowsSandboxUserStatusAsync, getWindowsSandboxCaCert, ensurePersistentWindowsCa, verifyWindowsWfpEgress, resolveSrtWin, WindowsSandboxError, DEFAULT_WINDOWS_PROXY_PORT_RANGE, } from './windows-sandbox-utils.js';
...
import { matchesDomainPattern, matchesDomainPatternWithPort, stripDomainPatternPort, } from './domain-pattern.js';
...
const commandTextsById = new Map();
function registerCommandText(command, options) {
 ...
 */
function resolveCommandText(decodedId) {
 ...
 */
function recordProxyViolation(line, encodedCommand) {
...
async function filterNetworkRequest(port, host, sandboxAskCallback, encodedCommand) {
    ...
    for (const deniedDomain of config.network.deniedDomains) {
        if (matchesDomainPatternWithPort(canonicalHost, port, deniedDomain)) {
            ...
            return denied(config.network.deniedDomainReasons?.[deniedDomain] ??
 ...
 */
function buildBodyCredentialInjector() {
    if (!config?.credentials)
        return undefined;
    return destHost => sentinelRegistry.sentinelsForHost(destHost, matchesDomainPattern);
}
/**
...
function buildSigv4Planner() {
    ...
    return (method, requestTarget, headers, destHost) => createSigv4Planner(awsPairRegistry, config?.credentials?.sigv4, matchesDomainPattern)(method, requestTarget, headers, destHost);
}
...
async function startMuxProxyServer(sandboxAskCallback, portRange) {
    ...
    httpProxyServer = createHttpProxyServer({
        ...
        onFilterRequestDenied: ({ method, url, reason, encodedCommand }) => {
            recordProxyViolation(`deny http-request ${method} ${redactUrlForViolation(url)} (${reason})`, encodedCommand);
        },
    ...
    socksProxyServer = createSocksProxyServer({
        filter: (port, host, encodedCommand) => filterNetworkRequest(port, host, sandboxAskCallback, encodedCommand),
        parentProxy,
        proxyAuthToken,
        probeUnauthenticated: async (port, host) => {
            ...
            for (const entry of config.network.deniedDomains) {
                if (matchesDomainPatternWithPort(canonical, port, entry)) {
                    const reason = config.network.deniedDomainReasons?.[entry] ??
                        'host is on the deny list';
                    recordProxyViolation(`deny network-outbound ${host}:${port} (${reason})`, undefined);
...
async function initialize(runtimeConfig, sandboxAskCallback, enableLogMonitor = false) {
    ...
    const useWindowsPersistentCa = getPlatform() === 'windows' &&
    ...
    if (enableLogMonitor && getPlatform() === 'macos') {
        logMonitorShutdown = startMacOSSandboxLogMonitor(sandboxViolationStore.addViolation.bind(sandboxViolationStore), config.ignoreViolations, resolveCommandText);
    ...
    if (getPlatform() === 'windows') {
        ...
        if (!u.provisioned || !u.credPresent) {
            config = undefined;
            throw new WindowsSandboxError('not_provisioned', `Windows sandbox user is not provisioned (user=` +
        ...
        if (tlsTerminate && mitmCA && !useWindowsPersistentCa) {
            ...
            if (!installed) {
                config = undefined;
                throw new WindowsSandboxError('trust_ca_not_installed', `tlsTerminate on Windows requires the sandbox to be ` +
            ...
            if (installed.thumb !== sessionThumb) {
                config = undefined;
                throw new WindowsSandboxError('trust_ca_thumbprint_mismatch', `tlsTerminate on Windows: the sandbox's installed CA ` +
...
function getCredentialRestrictions(credentials, allowedDomains) {
    ...
    const { setEnvVars, degradeToUnsetNames } = buildMaskedEnvVars(credentials.envVars ?? [], defaultInjectHosts, sentinelRegistry);
    ...
    registerAwsPairs(credentials.envVars ?? [], credentials.awsPairs, defaultInjectHosts, setEnvVars, awsPairRegistry);
    ...
    const { binds: maskedFileBinds, degradeToDenyPaths } = buildMaskedFileBinds(files, defaultInjectHosts, sentinelRegistry, maskedFileStore);
    return {
 ...
 */
function getGitSafeDirectories(customConfig) {
    return [
...
async function wrapWithSandboxArgv(command, binShell, customConfig, abortSignal, cwd, options) {
    ...
    if (platform === 'windows') {
        ...
        if (!fsCfg?.disabled) {
            ...
            if (rawRead.length > 0 || rawWrite.length > 0) {
                ...
                const expand = expandWindowsFsPaths;
                perExecDenyRead = expand(rawRead, { mode: 'deny' }).filter(p => !sessRead.has(p));
                perExecDenyWrite = expand(rawWrite, { mode: 'deny' }).filter(p => !sessRead.has(p) && !sessWrite.has(p));
            }
        }
    ...
    if (typeof binShell === 'object') {
        throw new Error('binShell object form is Windows-only; pass a shell path string ' +
            'on macOS/Linux');
    }
